# Escala de Usabilidade do Sistema (SUS)

**SISTEMA:** Interface de jogo - Zelda: Tears of the Kingdom

## Instruções

Preencha a coluna **"Resposta (1–5)"**. A coluna **"Ajuste"** é calculada automaticamente conforme a metodologia SUS:

- Itens ímpares (1, 3, 5, 7, 9): `(resposta − 1)`
- Itens pares (2, 4, 6, 8, 10): `(5 − resposta)`
- A pontuação final é a **soma dos ajustes × 2,5**, variando de 0 a 100.

### Legenda de resposta

| Valor | Resposta |
|---:|---|
| 1 | Discordo totalmente |
| 2 | Discordo parcialmente |
| 3 | Neutro |
| 4 | Concordo parcialmente |
| 5 | Concordo totalmente |

## Questionário SUS

| Item | Resposta (1–5) | Ajuste (automático) |
|---|---:|---:|
| 1. Eu acho que gostaria de usar este sistema com frequência. | 4 | 3 |
| 2. Eu achei o sistema desnecessariamente complexo. | 1 | 4 |
| 3. Eu achei o sistema fácil de usar. | 3 | 2 |
| 4. Eu acho que precisaria de ajuda de uma pessoa com conhecimentos técnicos para conseguir usar o sistema. | 1 | 4 |
| 5. Eu achei que as várias funções do sistema estavam bem integradas. | 3 | 2 |
| 6. Eu achei que houve muita inconsistência neste sistema. | 1 | 4 |
| 7. Eu imagino que a maioria das pessoas aprenderia a usar este sistema rapidamente. | 4 | 3 |
| 8. Eu achei o sistema muito difícil de usar. | 2 | 3 |
| 9. Eu me senti confiante ao usar o sistema. | 3 | 2 |
| 10. Eu precisei aprender várias coisas novas antes de conseguir usar o sistema. | 1 | 4 |

## Resultado

- **Soma dos ajustes:** 31
- **Pontuação SUS:** 77,5
- **Classificação qualitativa:** Boa (B)
- **Comparação com média (68):** Acima da média (SUS ≥ 68)

## Interpretação da Pontuação SUS

| Pontuação SUS | Interpretação | Classificação
|---:|---|---|
| 85 – 100 | Excelente usabilidade | “A” – Altamente aceitável |
| 70 – 84 | Boa usabilidade | “B” – Aceitável |
| 50 – 69 | Média / aceitável, mas com problemas | “C” – Marginal |
| < 50 | Baixa usabilidade | “D/E” – Inaceitável |
